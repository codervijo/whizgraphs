...
} from "@/components/site/InfographicPreview";
import {
  TEMPLATES,
  trafficSeries,
  topPages,
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard — WhizGraphs" },
      { name: "description", content: "Create SEO infographics from GSC-style data. Pick a template, edit branding, preview live." },
    ],
  }),
  component: Dashboard,
});
  ...
  return (
    <aside className="md:sticky md:top-20 md:self-start">
      ...
      <div className="mt-3 hidden rounded-xl border border-dashed border-border p-3 text-xs text-muted-foreground md:block">
        ...
        <p className="mt-1">Using mock GSC data. <Link to="/pricing" className="text-primary">Upgrade</Link> for live sync.</p>
      </div>
    </aside>
  );
}
  ...
  return (
    <Card title="Brand defaults" icon={Palette}>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Multi-brand kits for client work land on the <Link to="/pricing" className="text-primary">Agency plan</Link>.
      </p>
    </Card>
  );
}
